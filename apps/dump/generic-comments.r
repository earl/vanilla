	...
	format-entry: func [sn mode /local age e tc c current-comment user-ids users commenters template] [
		...
		either (= 0 count-for sn) [
			...
			for i 1 count-for sn 1 [
				...
				if error? try
					[ u: users-get-by-id space-meta-get current-comment "last-editor-id" u: u/get 'name]
					[ u: "unknown" ]
				c: replace/all c "[author]" u
				if error? try
					[ a: age/date-to-age space-meta-get current-comment "last-store-date" ]
					[ a: "a long time ago" ]
				c: replace/all c "[age]" a

				either (not = user none) [
					either (= (user/get 'name) u) [
						c: replace/all c "[edit-button]" rejoin [{<input type="button" value="edit" style="border-width:1px;background-color:#f1f1f1;border-color:#aaaaaa;font-family:Verdana;font-size:11px;" onClick="document.location.href='{vanilla-edit-url}} current-comment {'">}]
						] [
