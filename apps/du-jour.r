...
make object! [
	...
	format-entry: func [d mode /local age comments current-name tc c user-ids users commenters] [
		; two modes: 'individual and 'comments
		; [date] [fulldate] [time] [author] [age] [#comments] [commenters] [comments] [content]
		...
		if error? try [u: users-get-by-id space-meta-get current-name "last-editor-id" u: u/get 'name] [u: "unknown"]
		...
		if (not space-exists? join "comments-" current-name) [
			space-store join "comments-" current-name
				rejoin ["{!comments:" current-name "}"]
				]

		e: replace/all e "[#comments]" 
			rejoin ["<a href={script-name}?selector=display&snip=comments-" current-name ">"
		...
		if (not = find e "[comments]" none) [
			either (= 0 comments/count-for current-name) [
				e: replace/all e "[comments]" rejoin [space-get "comments-header" "<i>no comments</i><br><br>" "{!comments-new-form}" space-get "comments-footer"]
				] [
				tc: make string! 1024
				...
				e: replace/all e "[comments]" rejoin [space-get "comments-header" tc "{!comments-new-form}" space-get "comments-footer"]
				]
			]
